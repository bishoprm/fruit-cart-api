...
export const CalculateCartTotal = (
  ...
  cartItemArray.forEach((item: PriceObject) => {
    ...
    const { subtotal, taxes, total }: PriceObject = calculateTotalPerItem(item[fruitName], priceList[fruitName]);
...
const calculateTotalPerItem = (item: number, itemPrice: number): PriceObject => {
  const subtotal = item * itemPrice;
  const taxes = calculateTax(subtotal);
  return { subtotal, taxes, total: subtotal + taxes };
};

const formatCartPrices = (total: CartObject): CartObjectInUSD => {
  return {
